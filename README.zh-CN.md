...
一套自托管的双语博客平台,支撑着 [xiudou.site](https://xiudou.site) 的日常运行。
...
npm run test             # node --test,43 个测试文件
npm run lint             # eslint
npm run db:generate      # 按 schema 变更生成迁移
npm run db:migrate       # 执行迁移
npm run db:studio        # Drizzle Studio
npm run screenshots      # 重新抓取 README 截图
npm run content:rerender # schema 或渲染管线变更后重跑全部文章 HTML
```

测试集中在 `lib/**/*.test.ts`,覆盖认证、限流、令牌、密码策略、权限判定、Markdown 管线和 i18n 字典等纯逻辑模块,用 Node 内置 test runner,不需要起数据库。

### 改动翻译

`messages/zh.json` 和 `messages/en.json` 必须保持同步,四个测试在守这条线:key 集合一致、不许有空文案、占位符与富文本标签匹配、英文文件里不许残留中文。漏翻一条 key 的后果通常是某个冷门页面运行时炸掉,不测出来根本发现不了。

服务端动作返回的是字典 key 而不是文案——动作自己没有 locale,由表单在渲染时翻译。
...
app/
  [locale]/
    (site)/          前台:首页、文章、归档、分类标签系列、账户中心
    (auth)/          登录、注册、验证邮箱、找回密码、MFA
  (backend)/admin/   后台:文章、媒体库、设置——不参与 i18n
  api/               REST 端点
  internal/jobs/     调度器专用,需 JOB_SECRET
i18n/                路由、导航、字典加载、hreflang
